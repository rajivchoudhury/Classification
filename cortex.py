...
class Cortex:
	...
	async def getAuthorizationToken2(self):
		request = self.generateRequest("authorize", clientId = self.clientId, clientSecret = self.secretId, debit = 100000)
		response = await self.sendRequest(request)
		print(response)
		self.authorizationToken = response["result"]["cortexToken"]
		print("A Cortex access token has been generated")
	...
	async def actionOnHeadset(self, mode, headset):
		if self.headsetId == None:
			return "Headset not paired"
		else:
			request = self.generateRequest("controlDevice", command = mode, headset = headset)
			response = await self.sendRequest(request)
			if mode == "connect":
				self.headsetStatus = "connected"
				self.headsetId = headset
	...
	# To get license info
	async def getLicenseInfo(self):
		request = self.generateRequest("getLicenseInfo", cortexToken = self.authorizationToken)
		response = await self.sendRequest(request)
		return response
	...
	async def mentalCommandActiveAction(self, status, profile):
		request = self.generateRequest("mentalCommandActiveAction", cortexToken = self.authorizationToken, profile = profile, status = status)
		response = await self.sendRequest(request)
		print(response)
		return response
